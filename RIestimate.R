...
RIestimate <- function(Y0,Y1,M,verbose=FALSE,alpha=0.05,method=c("binomial","fixed.margins"),statistic=c("means","ks","wilcox")) {
  ...
  if(!isBinary) {
    ...
    if(doWX) {
      out$wilcox.test<-wilcox.test(Yr, Yl, alternative = c("two.sided"), mu = 0,paired = FALSE, exact = NULL, correct = TRUE, conf.int = FALSE)
  ...
  if(exactFM) {
    ...
    if(doMeans) {
      mns<-stats[1,]
      out$meansFM.pval<-sum(abs(mns)>=abs(mn0))/ncomb
    }
    if(!isBinary){
      if(doKS) {
        kss<-stats[doMeans+1,]
    ...
    if(doMeans) {
      mns<-stats[1,]
      out$meansFM.pval<-sum(abs(mns)>=abs(mn0))/M
    }
    if(!isBinary){
      if(doKS) {
        kss<-stats[doMeans+1,]
  ...
  if(exactBN) {
    ...
    if(doMeans) {
      mns<-stats[1,]
      out$meansBN.pval<-sum(p[abs(mns)>=abs(mn0)])
    }
    if(!isBinary){
      if(doKS) {
        kss<-stats[doMeans+1,]
